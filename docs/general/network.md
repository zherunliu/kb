...
HTTP（HyperText Transfer Protocol）是基于请求/响应模型的应用层协议；协议本身是无状态的：单个请求不自动继承上一个请求的上下文，应用仍可以通过 Cookie、Session、Token 等机制建立有状态的业务会话
...
- **支持分块传输编码**：HTTP/1.1 可以使用 `Transfer-Encoding: chunked` 发送事先不知道总长度的响应，每块携带十六进制长度，最后以长度为 `0` 的块结束；HTTP/2 和 HTTP/3 使用自身的帧边界，不使用该编码
...
- HTTP/1.1 应用层队头阻塞：流水线请求的响应必须按请求顺序返回；实践中客户端通常不开启流水线，而是通过有限数量的并行 TCP 连接提高并发
- 协议开销大：每个请求会重复携带文本形式的 header，HTTP/1.1 没有像 HPACK/QPACK 这样的协议级头部压缩
...
- 恢复已有会话时可以使用 0-RTT 提前发送数据；0-RTT 数据存在重放风险，不适合无幂等保障的敏感操作
- QUIC 使用连接 ID 支持**连接迁移**，网络路径变化时（例如从 Wi-Fi 切换到蜂窝网络）不必仅因 IP 地址改变而重建连接

> - 浏览器根据服务端能力、ALPN、`Alt-Svc`、DNS HTTPS/SVCB 记录和网络情况自动选择 HTTP 协议
> - 服务端控制：
>   - HTTP/2：配置 HTTPS，并让 TLS ALPN 提供 `h2`
>   - HTTP/3：监听 UDP 443、启用 QUIC/TLS 1.3，并通常通过响应头 `Alt-Svc: h3=":443"; ma=86400` 向客户端声明
...
DNS（Domain Name System）既是一套分层、分布式的命名数据库，也是一套查询该数据库的应用层协议。DNS 除了保存域名对应的 IP，还可以保存邮件服务器、域的权威服务器、证书签发限制等数据
...
- 检查 DNS 缓存顺序：
  ...
  2. 操作系统 DNS
  3. 本机 `/etc/hosts` 文件
- 如果上述都没有命中，开始执行**递归查询**：客户端请求本地 DNS 服务器（例如：家庭路由器，企业 DNS 服务器，运营商提供的 DNS 服务器），如果命中，则返回；如果未命中，则本地服务器执行**迭代查询**：
  1. 本地 DNS 服务器 -> 根 DNS 服务器（例：.）
  2. 本地 DNS 服务器 -> 顶级 DNS（TLD）服务器（例：.com）
  3. 本地 DNS 服务器 -> 权威域名服务器（例：阿里云解析）
  4. 本地 DNS 服务器缓存结果，并返回结果给客户端
- 使用 CDN 时，业务域名通常通过 CNAME 指向 CDN 提供的名称。递归解析器继续解析该目标名称，CDN 的权威 DNS 再根据位置、健康状态和容量等因素返回边缘节点地址

详细内容和终端命令见 [DNS](./dns.md)
...
4. 网络进程根据 HTTP 缓存规则判断能否直接复用新鲜响应；缓存不存在、已过期或要求重新验证时，才访问网络或向服务器发送条件请求
...
> postMessage 只能在父页面和嵌入的 iframe 之间通信，单独打开的 `toMessage.html` 是独立的页面，无法与父页面通信
...
1. 客户端发送 `ClientHello`：声明支持的 TLS 版本、密码套件、密钥交换参数，并通常携带 SNI（目标域名）和 ALPN（例如 `h2`、`http/1.1`）
2. 服务端发送 `ServerHello`：选择协议参数并返回自己的临时密钥交换参数，双方使用（EC）DHE 结果派生握手密钥，服务端的长期私钥不直接用于加密业务数据
3. 服务端在加密的握手消息中发送证书链、`CertificateVerify` 和 `Finished`，用证书对应的私钥证明自己控制该身份，并证明握手消息未被篡改
4. 客户端验证证书链是否能到达受信任根、证书域名是否匹配、时间是否有效、用途是否允许，以及握手签名是否正确
5. 客户端发送自己的 `Finished`；握手完成后，双方使用派生出的对称密钥和 AEAD 算法保护应用数据
...
数字签名（Digital Signature）用于验证数据是否被篡改，以及签名者是否持有对应的私钥。典型流程如下：

1. 计算摘要：发送者对原始数据进行哈希运算，得到固定长度的摘要
2. 生成签名：发送者使用私钥和签名算法对摘要执行签名运算，生成数字签名
3. 发送数据：发送者将原始数据和数字签名一起发送给接收者
4. 验证签名：接收者重新计算原始数据的摘要，并使用发送者的公钥和相同的签名算法验证签名；验证成功表示数据未被篡改，且签名由对应私钥生成
5. 确认身份：签名验证本身不能证明公钥属于谁，还需要通过证书链、预先安全分发公钥等方式确认公钥持有者的身份

数字签名可以提供完整性、来源认证，并在特定制度和密钥管理前提下支持不可否认性，常用于软件发布、数字合同和数字证书等
