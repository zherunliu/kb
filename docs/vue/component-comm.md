...
`props` 是使用频率最高的一种通信方式，子组件中使用宏函数 `defineProps` 定义自定义属性，常用于：**父 <=> 子**
...
用于普通 DOM 标签时，获取的是 DOM 节点；用于组件标签时，获取的是组件实例对象

> 使用 `<script setup>` 的组件默认是封闭的，父组件无法直接访问子组件内部的属性和方法，需要通过 `defineExpose` 显式暴露
